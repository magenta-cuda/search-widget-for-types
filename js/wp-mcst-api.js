...
(function( window, undefined ) {
	...
	var wp = window[space.name] = window[space.name] || {};
	wp.api = wp.api || new WP_API();
	wp.api.versionString = wp.api.versionString || space.versionString;
...
(function() {
  ...
  var wp = window[space.name];
	var wpApiSettings = window[space.name+"ApiSettings"] || {};
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		wpApiSettings = window[space.name+"ApiSettings"] || {};
	var wp = window[space.name] = window[space.name] || {};
	wp.api = wp.api || {};
	...
	Endpoint = Backbone.Model.extend({
      ...
      mapping = wpApiSettings.mapping || space.mapping;
...
window[space.name].api.loadPromise.done( function() {
    deferred.resolve();
} );
